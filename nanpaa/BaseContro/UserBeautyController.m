...
@interface UserBeautyController () <CAAnimationDelegate> {
    CGFloat _allTime;
    UIImage *_tempImg;
...
@implementation UserBeautyController
...
- (void)dealloc {
    [[NSUserDefaults standardUserDefaults] setObject:@"userHeadIconNo" forKey:@"userHeadIconBOOL"];
...
- (void)setupUI {
    ...
    self.filterSwitch = ({
        UIButton *b = [UIButton buttonWithType:UIButtonTypeCustom];
        [b setBackgroundImage:[UIImage imageNamed:@"meiyanG"] forState:UIControlStateNormal];
        [b setBackgroundImage:[UIImage imageNamed:@"meiyanK"] forState:UIControlStateSelected];
    ...
    self.cameraSwitch = ({
        UIButton *b = [UIButton buttonWithType:UIButtonTypeCustom];
        [b setBackgroundImage:[UIImage imageNamed:@"jtqh"] forState:UIControlStateNormal];
        [b setBackgroundImage:[UIImage imageNamed:@"jtqh"] forState:UIControlStateSelected];
    ...
    self.recordButton = ({
        UIButton *b = [UIButton buttonWithType:UIButtonTypeCustom];
        [b setBackgroundImage:[UIImage imageNamed:@"camera_btn_cramre_normal_55x55_"] forState:UIControlStateNormal];
        [b setBackgroundImage:[UIImage imageNamed:@"camera_btn_cramre_normal_55x55_"] forState:UIControlStateHighlighted];
    ...
    self.recaptureButton = ({
        UIButton *b = [UIButton buttonWithType:UIButtonTypeCustom];
//        b.alpha = 1;
        [b setBackgroundImage:[UIImage imageNamed:@"marCancel"] forState:UIControlStateNormal];
...
- (void)beginRecord {
    ...
    if (![[[NSUserDefaults standardUserDefaults] objectForKey:@"userHeadIconBOOL"] isEqualToString:@"userHeadIconYES"]) {
...
- (void)endRecord {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"headIcon"]) {
                    // 储存到图片库,并且设置回调.
                    ...
                    [self.videoCamera capturePhotoAsImageProcessedUpToFilter:(self.filterSwitch.selected ? self.leveBeautyFilter : self.normalFilter) withCompletionHandler:^(UIImage *processedImage, NSError *error) {
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            NSDictionary *dic = @{@"img":_tempImg};
                            [weakSelf dismissViewControllerAnimated:YES completion:nil];
                            [[NSNotificationCenter defaultCenter] postNotificationName:@"changeheadImg" object:weakSelf userInfo:dic];
        ...
        if (_allTime < 1) {
            ...
            //                NSDictionary *dic = @{@"img":_tempImg};
            //                [weakSelf dismissViewControllerAnimated:YES completion:nil];
            //                [[NSNotificationCenter defaultCenter] postNotificationName:@"postPaishe" object:weakSelf userInfo:dic];
            //            });
            //        }];
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"Recording time too short please record again !" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alert show];
        }else {
            // 储存到图片库,并且设置回调.
            kWeakSelf
            [self.movieWriter finishRecordingWithCompletionHandler:^{
                [weakSelf createNewWritter];
                dispatch_async(dispatch_get_main_queue(), ^{
                    //                _avplayer = [AVPlayerLayer playerLayerWithPlayer:[AVPlayer playerWithURL:[NSURL fileURLWithPath:videoPath]]];
                    //                _avplayer.frame = weakSelf.view.bounds;
                    //                [weakSelf.view.layer insertSublayer:_avplayer above:weakSelf.cameraView.layer];
                    //                [_avplayer.player play];
                    [weakSelf dismissViewControllerAnimated:YES completion:nil];
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"postPaishe" object:weakSelf userInfo:nil];
...
- (void)createNewWritter {
    ...
    _movieWriter = [[GPUImageMovieWriter alloc] initWithMovieURL:[NSURL fileURLWithPath:self.moviePath] size:CGSizeMake(kCameraWidth, kCameraWidth) fileType:AVFileTypeMPEG4 outputSettings:self.videoSettings];
    /// 如果不加上这一句，会出现第一帧闪现黑屏
...
//        NSDictionary *dic = @{@"takeImg":_tempImg};
//        [[NSNotificationCenter defaultCenter] postNotificationName:[[NSUserDefaults standardUserDefaults] objectForKey:@"postHome"] object:self userInfo:dic];

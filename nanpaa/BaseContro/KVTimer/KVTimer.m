...
#define DEGREES_TO_RADIANS(angle) ((angle) / 90.0 * M_PI)
...
- (void)setupView{
    ...
    self.pinStyle = !self.pinStyle ? [KVStyle initWithFillColor:RGB(212, 39, 82) strokeColor:[UIColor whiteColor] lineWidth:2 radius:dR] : self.pinStyle;
    self.circleStyle = !self.circleStyle ? [KVStyle initWithFillColor:[UIColor clearColor] strokeColor:[UIColor colorWithWhite:1 alpha:0.1] lineWidth:2 radius:R] : self.circleStyle;
    self.lineStyle = !self.lineStyle ? [KVStyle initWithFillColor:[UIColor clearColor] strokeColor:RGB(212, 39, 82) lineWidth:2 radius:R] : self.lineStyle;
...
- (void)panGestureRecognized:(UIPanGestureRecognizer *)sender
{
    ...
    self.newAngleInRadians = atan2f(translation.y - self.frame.size.height/2, translation.x - self.frame.size.width/2);
    NSLog(@"%@",self.timeLabel.text);
    NSLog(@"%f",fabs(self.oldAngleInRadians));
    NSLog(@"%f",fabs(self.newAngleInRadians));

    NSLog(@"%f",fabs(self.oldAngleInRadians) - fabs(self.newAngleInRadians));
    
    if ([self.timeLabel.text isEqualToString:@"0"] && fabs(self.oldAngleInRadians) == fabs(self.newAngleInRadians)) {
        
    }else if ([self.timeLabel.text isEqualToString:@"800"] && fabs(self.oldAngleInRadians) < fabs(self.newAngleInRadians)){
        
    }else{
        float angle = self.newAngleInRadians + M_PI / 360;
...
- (void)showLabelTime{
    ...
    self.oldAngleInRadians = self.newAngleInRadians;
    self.timeLabel.text = [NSString stringWithFormat:@"%li", (long)self.currentTime * 5];
}

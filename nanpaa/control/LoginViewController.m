...
@interface LoginViewController ()<SDCycleScrollViewDelegate,UIScrollViewDelegate,MBProgressHUDDelegate>
...
@property (nonatomic, assign) CGFloat imgWithBtnWidth;
@end
...
- (void)viewDidLoad {
    ...
    if (kScreenWidth < 375) {
        self.imgWithBtnWidth = 45;
    }else{
        self.imgWithBtnWidth = 70;
    }
...
- (void)initUI {
    ...
    [_emailBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(wSelf.logoView);;
    ...
    [_faceImg mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(wSelf.fbBtn.mas_left).offset(wSelf.imgWithBtnWidth);
    ...
    //    self.loginBtn.layer.shadowOpacity = 0.5;// 阴影透明度
    //    self.loginBtn.layer.shadowColor = [UIColor blackColor].CGColor;// 阴影的颜色
    //    self.loginBtn.layer.shadowRadius = 4;// 阴影扩散的范围控制
    //    self.followBtn.layer.shadowOffset = CGSizeMake(1, 4);// 阴影的范围
}
...
-(void)fbLogin {
    ...
    [self.fbBtn addSubview:activityIndicator];
    
    [activityIndicator mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(wSelf.fbBtn.mas_right).offset(- wSelf.fbBtn.size.width / 4 + wSelf.fbBtn.size.height / 2);
        make.centerY.equalTo(wSelf.fbBtn);
        make.height.width.offset(wSelf.fbBtn.size.height / 2);
    }];
    ...
    [ShareSDK getUserInfo:SSDKPlatformTypeFacebook onStateChanged:^(SSDKResponseState state, SSDKUser *user, NSError *error)
     {
         if (state == SSDKResponseStateSuccess)
         {
             ...
             [manager POST:Url parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
                 ...
                 //                 [wSelf.navigationController pushViewController:mainVC animated:YES];

...
-(void)textFieldDidChange :(UITextField *)theTextField{
    ...
    [[DJHttpApi shareInstance] POST:searchCountryUrl dict:dict succeed:^(id data) {
        if ([[NSString stringWithFormat:@"%@",data[@"rspCode" ]] isEqualToString:@"10012"]) {
            ...
            [wSelf.listTeamsArray removeAllObjects];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSDictionary *userLoction = @{@"latitude":model.latitude,
                                  @"longitude":model.longitude};
    [[NSUserDefaults standardUserDefaults] setObject:userLoction forKey:@"userLoction"];
    //    PositionCityUrl

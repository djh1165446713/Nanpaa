...
//    apnsCertName:推送证书名（不需要加后缀），详细见下面注释。
    ...
    // 测试使用
    if (canShow || [[NSUserDefaults standardUserDefaults] objectForKey:@"user"] == nil) {
        NewFeatureModel *model1 = [NewFeatureModel model:[UIImage imageNamed:@"launchOne"]];
        NewFeatureModel *model2 = [NewFeatureModel model:[UIImage imageNamed:@"launchTwo"]];
        NewFeatureModel *model3 = [NewFeatureModel model:[UIImage imageNamed:@"launchThree"]];
        self.window.rootViewController = [CoreNewFeatureVC newFeatureVCWithModels:@[model1,model2,model3] enterBlock:^{
            NSLog(@"回调结束");
...
#pragma mark - 获取app crash 日志
//- (void)getAppCrash{
//    NSSetUncaughtExceptionHandler(&)
//}
//
//void UncaughtExceptionHandler(nsex){
//    
//}

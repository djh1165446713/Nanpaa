...
@property (nonatomic, strong) NSMutableArray *dataArrDM;
...
- (void)viewDidLoad {
    ...
    _dataArrDM = [NSMutableArray array];
...
- (void)loadData {
    ...
    NSDictionary *par1 =@{@"token":par[@"token"],
                          @"userid":par[@"userid"]};
    [[DJHttpApi shareInstance] POST:GetFollowUrl dict:par1 succeed:^(id data) {
        NSLog(@"%@:---%@",self,data);
...
- (void)loadDataDM {
    ...
    NSDictionary *dictDM = @{@"token":userDict[@"token"],
        ...
        if ([[NSString stringWithFormat:@"%@",data[@"rspCode" ]] isEqualToString:@"10012"]) {
            ...
            if (data[@"rspObject"] != nil) {
                for (NSDictionary *dic in data[@"rspObject"]) {
                    ...
                    [model setValuesForKeysWithDictionary:dic];
                    [wSelf.dataArrDM addObject:model];
                }
            }
        }

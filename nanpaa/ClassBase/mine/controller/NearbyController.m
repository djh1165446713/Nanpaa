...
- (void)initUI{
    ...
    flowLayout.itemSize = CGSizeMake(79, 150);
...
- (void)loadData {
    ...
    NSDictionary *dic = @{@"latitude":_location[@"wei"],
                          @"longitude":_location[@"jing"],
...
- (void)isOpenLcationGps{
    
    if ([CLLocationManager authorizationStatus] == kCLAuthorizationStatusDenied) {
        _location = [[NSUserDefaults standardUserDefaults] objectForKey:@"userLoction"];
    }else{
        _location = [[NSUserDefaults standardUserDefaults] objectForKey:@"LocationInfo"];
...
-(UIEdgeInsets)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout insetForSectionAtIndex:(NSInteger)section{
    ...
    return UIEdgeInsetsMake(0, self.leftWidth, 0, self.leftWidth);
